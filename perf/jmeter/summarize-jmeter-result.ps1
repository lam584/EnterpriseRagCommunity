param(
  ...
  [double]$P90Seconds = [double]::NaN,
  [double]$AvgSeconds = [double]::NaN,
  [double]$SuccessRateOverride = [double]::NaN,
  [double]$TpsOverride = [double]::NaN
)
...
if (-not [string]::IsNullOrWhiteSpace($ResultsDir)) {
  ...
  if ([double]::IsNaN($P90Seconds) -or [double]::IsNaN($AvgSeconds) -or [double]::IsNaN($SuccessRateOverride) -or [double]::IsNaN($TpsOverride)) {
    throw "ResultsDir is empty, so P90Seconds, AvgSeconds, SuccessRateOverride and TpsOverride are required."
  }
}
...
$row = [ordered]@{
  ...
  $headers.success = ("{0:N2}%" -f $SuccessRateOverride)
  $headers.tps = $TpsOverride

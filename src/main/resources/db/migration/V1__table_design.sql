...
CREATE TABLE hot_scores (
                            ...
                            score_30d DOUBLE NOT NULL DEFAULT 0 COMMENT '30天热度分',
                            score_90d DOUBLE NOT NULL DEFAULT 0 COMMENT '3个月热度分',
                            score_180d DOUBLE NOT NULL DEFAULT 0 COMMENT '半年热度分',
                            score_365d DOUBLE NOT NULL DEFAULT 0 COMMENT '1年热度分',
...
CREATE TABLE retrieval_hits (
                                ...
                                hit_type ENUM('BM25','VEC','RERANK','COMMENT_VEC','AGG','POST') NOT NULL COMMENT '命中类型',
...
CREATE TABLE context_windows (
                                 ...
                                 budget_tokens INT NULL COMMENT '上下文预算Token数',
                                 selected_items INT NULL COMMENT '入选条数',
                                 dropped_items INT NULL COMMENT '剔除条数',
...
CREATE TABLE moderation_llm_config (
  id BIGINT UNSIGNED PRIMARY KEY AUTO_INCREMENT COMMENT '主键ID',
  multimodal_prompt_code VARCHAR(128) NULL COMMENT '多模态审核提示词编码',
...
CREATE TABLE moderation_confidence_fallback_config (
  ...
  chunk_threshold_chars INT NOT NULL DEFAULT 20000 COMMENT '分片触发阈值(字符数)',
  chunk_llm_reject_threshold DECIMAL(6,4) NOT NULL DEFAULT 0.7500 COMMENT '分片LLM拒绝阈值',
  chunk_llm_human_threshold DECIMAL(6,4) NOT NULL DEFAULT 0.5000 COMMENT '分片LLM人工阈值',
  thresholds_json JSON NULL COMMENT '扩展阈值配置(JSON)',
  version INT NOT NULL DEFAULT 0 COMMENT '乐观锁版本',
  updated_at DATETIME(3) NOT NULL DEFAULT CURRENT_TIMESTAMP(3) ON UPDATE CURRENT_TIMESTAMP(3) COMMENT '更新时间',
  updated_by BIGINT UNSIGNED NULL COMMENT '更新人用户ID',
...
CREATE TABLE qa_message_sources (
  ...
  comment_id BIGINT UNSIGNED NULL COMMENT '评论ID（评论区来源）',
  ...
  KEY idx_qms_comment (comment_id),
  ...
  CONSTRAINT fk_qms_comment FOREIGN KEY (comment_id) REFERENCES comments(id) ON DELETE SET NULL
...
CREATE TABLE access_logs (
    ...
    KEY idx_access_archived_created_id (archived_at, created_at, id),
    KEY idx_access_archived_user_created (archived_at, user_id, created_at, id),
    KEY idx_access_archived_status_created (archived_at, status_code, created_at, id),
...
CREATE TABLE moderation_chunk_sets (
  id BIGINT UNSIGNED PRIMARY KEY AUTO_INCREMENT COMMENT '主键ID',
  queue_id BIGINT UNSIGNED NOT NULL COMMENT '关联审核队列ID',
  case_type VARCHAR(16) NOT NULL COMMENT '案件类型(TEXT/IMAGE/MIXED)',
  content_type VARCHAR(16) NOT NULL COMMENT '内容类型(POST/COMMENT)',
  content_id BIGINT UNSIGNED NOT NULL COMMENT '内容ID',
  status VARCHAR(16) NOT NULL COMMENT '分片集合状态',
  chunk_threshold_chars INT NULL COMMENT '触发分片阈值(字符数)',
  chunk_size_chars INT NULL COMMENT '单分片目标字符数',
  overlap_chars INT NULL COMMENT '分片重叠字符数',
  total_chunks INT NOT NULL DEFAULT 0 COMMENT '分片总数',
  completed_chunks INT NOT NULL DEFAULT 0 COMMENT '已完成分片数',
  failed_chunks INT NOT NULL DEFAULT 0 COMMENT '失败分片数',
  memory_json LONGTEXT NULL COMMENT '运行记忆快照(JSON)',
  config_json LONGTEXT NULL COMMENT '分片配置快照(JSON)',
...
CREATE TABLE moderation_chunks (
  id BIGINT UNSIGNED PRIMARY KEY AUTO_INCREMENT COMMENT '主键ID',
  chunk_set_id BIGINT UNSIGNED NOT NULL COMMENT '所属分片集合ID',
  source_type VARCHAR(16) NOT NULL COMMENT '来源类型(TEXT/IMAGE/OCR)',
  source_key VARCHAR(64) NOT NULL COMMENT '来源唯一键',
  file_asset_id BIGINT UNSIGNED NULL COMMENT '关联文件资源ID',
  file_name VARCHAR(191) NULL COMMENT '文件名快照',
  chunk_index INT NOT NULL COMMENT '分片序号(从0开始)',
  start_offset INT NOT NULL COMMENT '起始偏移量(含)',
  end_offset INT NOT NULL COMMENT '结束偏移量(不含)',
  status VARCHAR(16) NOT NULL COMMENT '分片状态',
  attempts INT NOT NULL DEFAULT 0 COMMENT '处理尝试次数',
  last_error VARCHAR(1000) NULL COMMENT '最近一次错误信息',
  model VARCHAR(64) NULL COMMENT '执行模型名称',
  verdict VARCHAR(16) NULL COMMENT '分片裁决结果',
  confidence DECIMAL(5,4) NULL COMMENT '分片置信度',
  labels LONGTEXT NULL COMMENT '风险标签(JSON数组)',
  tokens_in INT NULL COMMENT '输入Token数',
  tokens_out INT NULL COMMENT '输出Token数',
...
CREATE TABLE ai_chat_context_events (
  ...
  user_id BIGINT UNSIGNED NULL COMMENT '用户ID',
  session_id BIGINT UNSIGNED NULL COMMENT '会话ID',
  question_message_id BIGINT UNSIGNED NULL COMMENT '问题消息ID',
  kind VARCHAR(32) NOT NULL COMMENT '事件类型',
  reason VARCHAR(64) NOT NULL COMMENT '触发原因',
  target_prompt_tokens INT NULL COMMENT '目标提示词Token预算',
  reserve_answer_tokens INT NULL COMMENT '预留答案Token预算',
  before_tokens INT NOT NULL COMMENT '治理前Token数',
  after_tokens INT NOT NULL COMMENT '治理后Token数',
  before_chars INT NOT NULL COMMENT '治理前字符数',
  after_chars INT NOT NULL COMMENT '治理后字符数',
  latency_ms INT NULL COMMENT '治理耗时(毫秒)',
  detail_json LONGTEXT NULL COMMENT '事件详情(JSON)',
  created_at DATETIME NOT NULL COMMENT '创建时间',
    PRIMARY KEY (id),

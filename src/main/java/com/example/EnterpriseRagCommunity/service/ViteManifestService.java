...
public class ViteManifestService {
    ...
    public void init() {
        ...
        try (InputStream is = openManifestStream()) {
            if (is == null) {
                logger.warn("未找到 Vite manifest.json（classpath:/static/.vite/manifest.json 或 my-vite-app/dist/.vite/manifest.json）。前端资源可能未构建；可先执行 Gradle 的 processResources/build 以触发 Vite 构建。");
                return;
            }
    ...
    private InputStream openManifestStream() {
        InputStream cp = getClass().getResourceAsStream("/static/.vite/manifest.json");
        if (cp != null) return cp;
        try {
            Path p = Path.of("my-vite-app", "dist", ".vite", "manifest.json");

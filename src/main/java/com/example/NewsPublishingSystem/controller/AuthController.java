...
import com.example.NewsPublishingSystem.config.AdminSetupManager;
import com.example.NewsPublishingSystem.dto.AdministratorDTOs;
...
import com.example.NewsPublishingSystem.service.AdminPermissionService;
...
public class AuthController {
    ...
    public ResponseEntity<?> getCurrentAdmin(HttpServletRequest request) {
        ...
        if (admin.isPresent()) {
            // 使用DTO替代直接返回实体
            AdministratorDTOs responseDTO = AdministratorDTOs.fromEntity(admin.get());
    ...
    public ResponseEntity<?> registerInitialAdmin(@Valid @RequestBody AdministratorDTOs request) {
        // 检查是否需要初始设置
        ...
        try {
            ...
            superAdminPermission.setAllowEditUserProfile(true);
            superAdminPermission.setAllowEditProfile(true);
            superAdminPermission.setAllowEditOtherAdminProfile(true);
            // 设置新闻系统相关权限
            superAdminPermission.setCanManageTopics(true);
            superAdminPermission.setCanManageNews(true);
            // 移除不存在于数据库中的字段设置
            // superAdminPermission.setCanLendBooksToUser(true); // 删除此行
            // superAdminPermission.setCanReturnBooksForUser(true); // 删除此行
            ...
            AdministratorDTOs responseDTO = AdministratorDTOs.fromEntity(savedAdmin);

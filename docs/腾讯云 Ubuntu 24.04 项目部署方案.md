...
# 克隆代码（请将下面的 URL 替换为您实际的仓库的 SSH 地址）
# 格式通常是：git@github.com:您的用户名/仓库名
git
...
项目依赖 Elasticsearch。我们将使用 Docker 安装，并配置 4.5GB 内存和 API Key 认证。
1. **安装 Docker**
   ```bash
   # 安装 Docker
...
> 如果显示 `BUILD SUCCESSFUL`，说明构建成功。生成的文件`build/libs/` 目录下。
---

## 第十步：配置 Nginx (Web 服务器

我们需。Nginx 来对外提供服务：它负责把用户的请求分发给前端页面或后端 API。
1. **安装 Nginx**：
```bash
   sudo apt install nginx -y
   ```

2. **创建配置文件**：   我们将使。`nano` 编辑器创建一个新的配置文件：   
```bash
   sudo nano /etc/nginx/sites-available/enterprise-rag
   ```

3. **粘贴以下内容**：
   > 请将下面。`/home/ubuntu/EnterpriseRagCommunity` 替换为您实际的项目路径。如果您使用的是 ubuntu 默认用户，通常路径就是这个。如果不确定，可以用 `pwd` 命令查看当前路径。
...
5. **配置 SSL 证书（HTTPS，Nginx）**
   ...
   > 注意：公。CA 证书一般要求域名；如果您只有公网 IP，没有域名，浏览器访问会出现证书不受信任属于正常现象。
   **5.1 上传并放置证书文件**
   ...
   > 如果您下载的是压缩包（例。`cert.zip`），可以先上传到服务器后解压，再按上面方法 `cp` 到 `/etc/nginx/ssl/...`。   
   ```bash
   ...
   **5.3 验证 HTTPS 与重启**
   ...
   ### 常见错误：如果 80 端口出现 500，但 8099 正常
      ...
      方法一：修改目录权限（推荐）：
      给 Nginx 用户（通常。www-data）赋予读取和执行权限：      
      ```bash
      # 1. 确保父目录有执行权限（必须！）：
...
为了让后端程序在后台运行且开机自动启动，我们创建一个系统服务器
1. **创建服务文件**。   
```bash
...
- 未配置 SSL：确保**TCP 80 端口**开放。 - 已配。SSL：确保 **TCP 443 端口** 开放（若启用了 80 转 443 跳转，也需开放 80）。
---
...
2. **Elasticsearch 配置**。   - **主机**: `localhost`
   - **端口**: `9200`
   - **协议**: `http` (因为我们在 Step 7 中关闭了 SSL)
   - **API Key**: 填入 **第七步** 中生成的 `encoded` 字符串。3. **其他服务**。   - 根据向导提示配置邮件服务 (SMTP) 。AI 模型密钥。
...
- **查看实时应用日志**（非常重要，用于排查报错）：
  ```bash
  # -f 表示 follow (实时刷新)。u 指定服务器  sudo journalctl -u enterprise-rag -f
  ```
  > 按 `Ctrl + C` 停止查看
...
当您在本地开发并 Push 了新代码。GitHub 后，服务器端需要执行以下步骤来应用更新：
```bash
# 1. 进入项目目录
cd ~/EnterpriseRagCommunity
...
cd my-vite-app
npm install  # 以防有新的依。npm run build
cd ..        # 回到根目录# 前端构建完成后立即生效（无需重启 Nginx，除非修改了 Nginx 配置文件：```
...
我们可以使用 `sed` 命令直接将配置文件中。`80` 端口替换。`8080`。请复制并执行以下命令：
...
## 第三步：在腾讯云控制台开放8080 端口

仅仅在服务器上修改是不够的，您还需要在腾讯云的防火墙（安全组）中放行 8080 端口。如果您同时把 HTTPS 端口改成了 8443，也需要放。`TCP:8443`。
...
- **关于备案**：如果您后续购买了域名并完成了备案，可以随时按照上述步骤将端口改为 80（只需将`sed` 命令中的 `8080` 。`80` 对调即可）。- **常用端口**：如果 8080 也被占用，您还可以尝试 8081、8088 等端口。
---
...
2. **使用新内存配置启动**（例如改为 2GB 堆内存，容器限额 2.5GB）：   ```powershell
   docker run -d --name elasticsearch `
     --net elastic `
     -p 9200:9200 `
     -e "discovery.type=single-node" `
     ...
     docker.elastic.co/elasticsearch/elasticsearch:9.2.4
   ```
   > 注：PowerShell 中换行符为反引号 `` ` ``，如果您使用 CMD 请改为 `^`，Bash 使用 `\`
...
1. **创建本地数据目录**
   ```powershell
   # 在当前目录下创建 es_data 文件   mkdir es_data
   ```
...
启动后，您可以进入容器检查JVM 参数是否生效：```powershell

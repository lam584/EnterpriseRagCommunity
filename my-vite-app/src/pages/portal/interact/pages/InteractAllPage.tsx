...
export default function InteractAllPage() {
  ...
  async function load() {
    ...
    setError(null);
    try {
      const page = await fetchNotifications({ page: 1, pageSize: 50 });
  ...
  return (
    <div className="space-y-3">
      <div className="flex items-center justify-between">
        <h3 className="text-lg font-semibold">全部通知</h3>
        <button
          type="button"
      ...
      <NotificationList items={items} onMarkRead={onMarkRead} onDelete={onDelete} showTypeBadge />
    </div>
  );
}

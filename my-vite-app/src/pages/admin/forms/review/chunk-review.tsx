...
function formatEnumZh(code: string | null | undefined, mapping: Record<string, string>): string {
  if (!code) return '—';
...
type BudgetConvergenceView = {
  ...
  rounds: Array<{ round: number; textTokenBudget: number; chunkSizeChars: number; estimatedTotalRequestTokens: number }>;
};

type TokenDiagnosticsView = {
  promptChars?: number;
  promptTokens?: number;
...
function parseBudgetConvergenceLog(v: unknown): BudgetConvergenceView | null {
...
function formatBudgetConvergenceSummary(v: unknown): string {
...
function parseTokenDiagnostics(v: unknown): TokenDiagnosticsView | null {
  if (!v || typeof v !== 'object') return null;
  const o = v as Record<string, unknown>;
...
const VISION_TOKEN_GRID_SIDE = 32 as const;
const VISION_TOKEN_PIXELS = VISION_TOKEN_GRID_SIDE * VISION_TOKEN_GRID_SIDE;
const VISION_TOKEN_LIMIT = 16_384;
const DEFAULT_DASHSCOPE_MAX_PIXELS = 2_621_440;
...
function toUrlString(v: unknown): string | null {
  const s = typeof v === 'string' ? v.trim() : '';
  return s ? s : null;
}

function createImageSizeProbe(opts: { concurrency: number }) {
...
const sharedImageSizeProbe = createImageSizeProbe({ concurrency: 6 });

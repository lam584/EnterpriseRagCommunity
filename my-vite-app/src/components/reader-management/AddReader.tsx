...
const AddReader: React.FC = () => {
    ...
    const validateForm = () => {
        ...
        if (formData.permissionId === 0) newErrors.permissionId = '请选择读者权限';
    ...
    const handleSubmit = async (e: React.FormEvent) => {
        ...
        try {
            ...
            // 更详细的成功消息，包含读者信息但不包含密码
            setMessage({
                type: 'success',
                text: `读者 "${newReader.account}" 添加成功！权限组: ${newReader.permission?.roles || ''}`
            });

            clearForm();
        } catch (err: any) {
            console.error('添加读者时出错:', err);
            setMessage({
                type: 'error',
                text: err.message || '添加读者失败，请检查填写内容或稍后重试'
            });
        } finally {
            setLoading(false);
        }
    ...
    return (
        <div className="flex">
            ...
            <div className="w-3/4 bg-white shadow-md rounded-md p-6">
                <h2 className="text-xl font-bold mb-4">添加新读者</h2>
                ...
                <form className="space-y-4" onSubmit={handleSubmit}>
                    <div className="grid grid-cols-2 gap-4">
                        ...
                        <div>
                            <label className="block mb-1">
                                读者权限 <span className="text-red-500">*</span>
                            </label>
                            <select
                                name="permissionId"

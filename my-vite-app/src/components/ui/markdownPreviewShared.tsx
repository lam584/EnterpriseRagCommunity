...
export function MarkdownPreviewContainer(props: { className?: string; children: ReactNode }) {
  const { className, children } = props;
  return <div className={`max-w-none text-sm text-gray-900 ${className ?? ''}`}>{children}</div>;
}

export function useMarkdownPreviewSetup(markdown: string, components?: Components) {
  ...
  const mergedComponents = useMemo<Components>(() => {
    const base: Components = {
      ...
      pre: ({ children, ...p }) => <CodeBlockPre {...p}>{children}</CodeBlockPre>,
    };

    return { ...base, ...(components ?? {}) };
  }, [components]);

  return { mergedComponents, normalizedMarkdown, sanitizeSchema };
}
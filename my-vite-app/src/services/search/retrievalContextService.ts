...
import { buildPageQuery } from '/services/shared/servicePagingUtils';
...
export async function adminListContextWindows(params?: {
  ...
  const res = await fetch(apiUrl(`/api/admin/retrieval/context/logs/windows?${buildPageQuery(params)}`), {
    method: 'GET',
    credentials: 'include',
  });
  const data: unknown = await res.json().catch(() => ({}));

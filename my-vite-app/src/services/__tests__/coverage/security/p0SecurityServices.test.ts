...
describe('p0SecurityServices', () => {
  ...
  it('passwordResetService getPasswordResetStatus parses booleans and sends csrf header', async () => {
    const { getPasswordResetStatus } = await import('/services/auth/passwordResetService');
  ...
  it('passwordResetService resetPasswordByTotp throws backend message on failure', async () => {
    const { resetPasswordByTotp } = await import('/services/auth/passwordResetService');
  ...
  it('emailChangeService verifyEmailChangePassword throws fallback message when missing', async () => {
    const { verifyEmailChangePassword } = await import('/services/auth/emailChangeService');
    const { replyJsonOnce } = installFetchMock();
  ...
  it('security2faPolicyAccountService verifyMyLogin2faPreferencePassword returns message', async () => {
    const { verifyMyLogin2faPreferencePassword } = await import('/services/auth/security2faPolicyAccountService');
    const { replyJsonOnce } = installFetchMock();
  ...
  it('security2faPolicyAdminService updateSecurity2faPolicySettings throws backend message', async () => {
    const { updateSecurity2faPolicySettings } = await import('/services/auth/security2faPolicyAdminService');
    const { replyJsonOnce } = installFetchMock();
  ...
  it('totpAdminService resetAdminUserTotp throws backend message', async () => {
    const { resetAdminUserTotp } = await import('/services/auth/totpAdminService');
    const { replyJsonOnce } = installFetchMock();
  ...
  it('totpAdminService get/update/query succeed and send csrf header', async () => {
    const { getTotpAdminSettings, updateTotpAdminSettings, queryAdminUserTotpStatus, resetAdminUserTotp } = await import('/services/auth/totpAdminService');
  ...
  it('totpAdminService get/update/query throw backend message and fallback', async () => {
    const { getTotpAdminSettings, updateTotpAdminSettings, queryAdminUserTotpStatus } = await import('/services/auth/totpAdminService');
    const { replyJsonOnce } = installFetchMock();
  ...
  it('passwordResetService send/reset succeed and throw fallback messages', async () => {
    const { sendPasswordResetEmailCode, resetPasswordByEmailCode, resetPasswordByTotp } = await import('/services/auth/passwordResetService');
    const { replyJsonOnce, lastCall } = installFetchMock();
  ...
  it('passwordResetService covers message type guards and json parse fallback', async () => {
    const { getPasswordResetStatus, sendPasswordResetEmailCode } = await import('/services/auth/passwordResetService');
  ...
  it('emailChangeService covers json parse fallback and message type guard', async () => {
    const { verifyEmailChangePassword, sendOldEmailVerificationCode, verifyOldEmailOrTotp } = await import('/services/auth/emailChangeService');
    const { replyOnce, replyJsonOnce } = installFetchMock();
  ...
  it('security2faPolicyAccountService get/update cover success and error paths', async () => {
    const { getMySecurity2faPolicy, updateMyLogin2faPreference } = await import('/services/auth/security2faPolicyAccountService');
    const { replyJsonOnce, lastCall } = installFetchMock();

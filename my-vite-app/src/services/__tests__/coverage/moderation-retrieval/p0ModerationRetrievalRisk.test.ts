...
describe('p0ModerationRetrievalRisk', () => {
  ...
  it('riskTagService createRiskTag uses slugify when slug missing', async () => {
    const { createRiskTag } = await import('/services/moderation/policy/riskTagService');
  ...
  it('moderationRulesService falls back to local storage on network error', async () => {
    const { adminListModerationRules } = await import('/services/moderation/policy/moderationRulesService');
  ...
  it('moderationRulesService create falls back to local storage on network error and persists', async () => {
    ...
    const { adminCreateModerationRule } = await import('/services/moderation/policy/moderationRulesService');
    const { fetchMock } = installFetchMock();
    fetchMock.mockRejectedValueOnce(new TypeError('Failed to fetch'));
  ...
  it('covers moderation* and retrieval* services happy paths', async () => {
    ...
    okJson();
    okJson();
    const { getModerationChunkReviewConfig, updateModerationChunkReviewConfig } = await import('/services/moderation/policy/moderationChunkReviewConfigService');
    await getModerationChunkReviewConfig();
    await updateModerationChunkReviewConfig({ enabled: true } as any);

    okJson();
    okJson();
    const { getFallbackConfig, updateFallbackConfig } = await import('/services/moderation/policy/moderationFallbackService');
    await getFallbackConfig();
    await updateFallbackConfig({ enabled: true } as any);

    okJson();
    okJson();
    const { adminGetLlmModerationConfig, adminUpsertLlmModerationConfig } = await import('/services/moderation/policy/moderationLlmService');
    await adminGetLlmModerationConfig();
    await adminUpsertLlmModerationConfig({ enabled: true } as any);

    okPage();
    okJson();
    const { adminListModerationQueue, adminGetModerationQueueDetail } = await import('/services/moderation/review/moderationQueueService');
    await adminListModerationQueue({ page: 0, size: 10 } as any);
    await adminGetModerationQueueDetail(1);

    okPage();
    okJson();
    const { adminListPipelineHistory, adminGetPipelineByRunId } = await import('/services/moderation/review/moderationPipelineService');
    await adminListPipelineHistory({ page: 0, size: 10 } as any);
    await adminGetPipelineByRunId(1);

    const { noopCsrfPing } = await import('/services/moderation/review/moderationPipelineService');
    await noopCsrfPing();

    okJson();
    const { getSamplesIndexStatus } = await import('/services/moderation/samples/moderationService');
    ...
    okJson();
    okJson();
    const { listSamples, createSample, updateSample, deleteSample, syncSample, syncSamplesIncremental } = await import('/services/moderation/samples/moderationEmbedSamplesService');
    ...
    const { adminListModerationChunkLogs, adminGetModerationChunkLogDetail, adminGetModerationChunkLogContent } = await import(
      '/services/moderation/review/moderationChunkReviewLogsService',
    );
    ...
    okJson();
    okJson();
    const { adminGetModerationPolicyConfig, adminUpsertModerationPolicyConfig } = await import('/services/moderation/policy/moderationPolicyService');
    await adminGetModerationPolicyConfig('POST' as any);
    await adminUpsertModerationPolicyConfig({ contentType: 'POST' } as any);

    okPage();
    okJson();
    const { adminListModerationReviewTraceTasks, adminGetModerationReviewTraceTaskDetail } = await import('/services/moderation/review/moderationReviewTraceService');
    ...
    okJson();
    okJson();
    const { adminGetCitationConfig, adminUpdateCitationConfig, adminTestCitation } = await import('/services/search/retrievalCitationService');
    ...
    const { adminGetContextClipConfig, adminUpdateContextClipConfig, adminTestContextClip, adminListContextWindows, adminGetContextWindow } =
      await import('/services/search/retrievalContextService');
    ...
    const { adminGetHybridRetrievalConfig, adminUpdateHybridRetrievalConfig, adminTestHybridRetrieval, adminTestHybridRerank, adminListHybridRetrievalEvents, adminListHybridRetrievalHits } =
      await import('/services/search/retrievalHybridService');
    ...
    const { adminListVectorIndices, adminCreateVectorIndex, adminUpdateVectorIndex, adminDeleteVectorIndex, adminBuildPostRagIndex, adminRebuildPostRagIndex, adminSyncPostRagIndex, adminGetRagAutoSyncConfig, adminUpdateRagAutoSyncConfig, adminTestQueryPostRagIndex } =
      await import('/services/search/retrievalVectorIndexService');

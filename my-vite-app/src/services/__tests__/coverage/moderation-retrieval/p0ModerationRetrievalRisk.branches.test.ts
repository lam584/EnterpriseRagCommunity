...
describe('p0ModerationRetrievalRisk branches', () => {
  ...
  it('retrievalHybridService throws backend message and falls back on json parse failure', async () => {
    const { adminGetHybridRetrievalConfig, adminUpdateHybridRetrievalConfig, adminListHybridRetrievalEvents } = await import('/services/search/retrievalHybridService');
  ...
  it('retrievalContextService includes from/to params and uses defaults', async () => {
    const { adminListContextWindows } = await import('/services/search/retrievalContextService');
  ...
  it('moderationChunkReviewLogsService builds query params and throws default error', async () => {
    const { adminListModerationChunkLogs } = await import('/services/moderation/review/moderationChunkReviewLogsService');
  ...
  it('moderationChunkReviewLogsService covers success, backend message, and detail/content branches', async () => {
    const { adminListModerationChunkLogs, adminGetModerationChunkLogDetail, adminGetModerationChunkLogContent } = await import('/services/moderation/review/moderationChunkReviewLogsService');
    const { replyJsonOnce, lastCall } = installFetchMock();
  ...
  it('retrievalCitationService and retrievalVectorIndexService cover error/default branches', async () => {
    ...
    const { adminGetCitationConfig } = await import('/services/search/retrievalCitationService');
    await expect(adminGetCitationConfig()).rejects.toThrow('m3');

    replyOnce({ ok: false, status: 400, jsonError: new Error('bad') });
    const { adminTestCitation } = await import('/services/search/retrievalCitationService');
    ...
    const { adminCreateVectorIndex, adminDeleteVectorIndex } = await import('/services/search/retrievalVectorIndexService');
  ...
  it('moderationEmbedSamplesService covers query filtering, 204 handling, and request failure branches', async () => {
    ...
    const { replyOnce, replyJsonOnce, lastCall } = installFetchMock();
    const { listSamples, createSample, deleteSample, syncSamplesIncremental } = await import('/services/moderation/samples/moderationEmbedSamplesService');

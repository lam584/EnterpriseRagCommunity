...
describe('p1AiLightServices', () => {
  ...
  it('covers ai* services happy paths and one error branch', async () => {
    ...
    const { adminProbeModel } = await import('/services/admin/ai/aiModelProbeAdminService');
  ...
  it('covers aiLangLabelService config and error fallback branches', async () => {
    ...
    const { getLangLabelGenConfig, suggestPostLangLabels } = await import('/services/ai/authoring/aiLangLabelService');

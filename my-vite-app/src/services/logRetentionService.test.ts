...
describe('logRetentionService', () => {
  ...
  it('adminGetLogRetentionConfig sends GET with credentials', async () => {
    const fetchMock = mockFetchResponseOnce({ ok: true, json: { enabled: true, keepDays: 7, mode: 'DELETE' } });

    const res = await adminGetLogRetentionConfig();

    expect(res).toEqual({ enabled: true, keepDays: 7, mode: 'DELETE' });
  ...
  it('adminUpdateLogRetentionConfig sends PUT with csrf header', async () => {
    const fetchMock = mockFetchResponseOnce({ ok: true, json: { enabled: false, keepDays: 1, mode: 'ARCHIVE_TABLE' } });
    const payload = { enabled: false, keepDays: 1, mode: 'ARCHIVE_TABLE' as const };
  ...
  it('adminUpdateLogRetentionConfig throws backend message or fallback', async () => {
    ...
    await expect(adminUpdateLogRetentionConfig({ enabled: true, keepDays: 2, mode: 'DELETE' })).rejects.toThrow('oops');

    mockFetchResponseOnce({ ok: false, status: 400, json: {} });
    await expect(adminUpdateLogRetentionConfig({ enabled: true, keepDays: 2, mode: 'DELETE' })).rejects.toThrow('更新配置失败');
  });

...
- 系统硬件要求与操作系统兼容性
- 软件依赖与前置条件检查（JDK 25+、MySQL、Elasticsearch/OpenSearch、Redis等）
- 环境变量与配置项映射
- 网络与防火墙配置建议
- 开发与生产差异配置及切换最佳实践
...
- 构建与工具链：Gradle + Spring Boot插件，Java Toolchain固定JDK 25，MySQL Connector/J与Flyway集成。
...
### JDK 25+ 安装与工具链
- 工程使用Java Toolchain固定JDK 25，确保构建与运行一致性。
- Gradle JVM参数与编码设置，保障日志与国际化输出稳定。
- 建议在CI/CD与生产环境均安装JDK 25并配置JAVA_HOME。
...
Build["build.gradle"] --> JDK["JDK 25 Toolchain"]
...
## 结论
本指南明确了企业级RAG社区平台的环境准备要点：JDK 25工具链、MySQL与Flyway迁移、Elasticsearch/OpenSearch动态鉴权、日志与监控、以及Nginx反向代理。生产环境应强化鉴权与网络隔离，严格控制管理端口暴露范围，并结合实际负载调优连接池与搜索客户端参数。

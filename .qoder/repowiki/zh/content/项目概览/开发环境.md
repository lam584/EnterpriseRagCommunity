...
## 核心组件
- 构建与运行
  - Gradle 9.x + Java 25 Toolchain，Spring Boot 3.5.x，Tomcat内嵌，DevTools热加载。
  - 默认打包为 WAR，禁用 JAR。
- 数据持久化
...
### 后端构建与运行配置
- JDK与工具链
  - 使用 Gradle Toolchain 指定 Java 25，确保编译与运行一致。
- Spring Boot与嵌入式容器
...
```mermaid
flowchart TD
Start(["Gradle 启动"]) --> Toolchain["解析 Java 25 Toolchain"]
...
### 本地开发环境搭建步骤
- JDK 与 Gradle
  - 安装 JDK 25，Gradle 9.x；Gradle Wrapper 已内置。
...
- Docker 容器化部署（思路）
  - 后端：基于 OpenJDK 25 镜像，复制 WAR 与配置，暴露 8099 端口。
